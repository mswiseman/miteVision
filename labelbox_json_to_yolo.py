...
def convert(file, zip=True):
    ...
    for img in tqdm(data, desc=f'Converting {file}'):
        ...
        for objects in img['Label']['objects']:
            if 'bbox' in objects:
                top, left, h, w = objects['bbox'].values()  # top, left, height, width
            else:
                continue
            xywh = [(left + w / 2) / width, (top + h / 2) / height, w / width, h / height]  # xywh normalized

            # class
            cls = objects['value']  # class name
            if cls not in names:
                names.append(cls)
...
if __name__ == '__main__':
    convert('export-2023-05-10T05_28_15.843Z.json')